...
func (r *CompRepositoriesImpl) Create(ctx *gin.Context, tx *gorm.DB, data database.Partners) (*string, *exceptions.Exception) {
...
func (r *CompRepositoriesImpl) FindByID(ctx *gin.Context, tx *gorm.DB, id string) (*database.Partners, *exceptions.Exception) {
	var partner_data database.Partners
...
func (r *CompRepositoriesImpl) FindByEmail(ctx *gin.Context, tx *gorm.DB, email string) (*database.Partners, *exceptions.Exception) {
	var partner_data database.Partners
...
func (r *CompRepositoriesImpl) CreateVerificationToken(ctx *gin.Context, tx *gorm.DB, id string) (*string, *exceptions.Exception) {
	delete_result := tx.Where("partner_id = ? AND category = ?", id, "email_verification").Delete(&database.PartnerTokens{})
	...
	token_data := database.PartnerTokens{
		PartnerID: id,
		Token:     token,
